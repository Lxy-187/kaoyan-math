  ...
  blocks: [
    ...
    { t: 'method', id: 'judge', title: '★ 判定方法：先看是具体的还是抽象的', c: String.raw`
      ...
      **具体向量（给出坐标）**：
      ...
      ==维数明显大于个数时（$3$ 个 $5$ 维向量之类），
      用[截短法](#/linear-algebra/vectors/independence?at=truncate)挑一个非零子式更快==，
      不必化整个阶梯形——但它==只能证无关，不能证相关==。
    ...
    { t: 'key', id: 'properties', title: '相关性的性质：三对口诀', c: String.raw`
      **前两条沿着两个互相垂直的方向变形，别混**：
      ...
      | 动的是什么 | 操作 | 对应口诀 |
      |---|---|---|
      | ==向量的个数==（横向：删向量） | 取子向量组 | ① 部分与整体 |
      | ==向量的维数==（纵向：删分量） | 延长 / 截短 | ② 延长与缩短 |

      =="删掉一个向量"和"删掉每个向量的一个分量"是完全不同的两件事==，
      结论也不同。**这是本节最容易串味的地方。**

      **翻成方程组的话，这两个轴就是==增删未知数==与==增删方程==**：
      加一个未知数是多一个列向量（个数轴），
      加一个方程是每个列向量长高一格（维数轴）。
      对照表见[两个轴在方程组这边是什么](#/linear-algebra/linear-systems/translation?at=axes-in-system)。
      ...
      **为什么"整体无关 $\Rightarrow$ 部分无关"**（反证，两行，值得会写）：
      设 $\alpha_1,\dots,\alpha_m$ 无关，而某个子组（不妨设为前 $s$ 个）相关，
      则存在不全为零的 $k_1,\dots,k_s$ 使
      $$k_1\alpha_1+\cdots+k_s\alpha_s=0.$$
      ==给剩下的 $\alpha_{s+1},\dots,\alpha_m$ 一律配系数 $0$==，就得到
      $$k_1\alpha_1+\cdots+k_s\alpha_s+0\cdot\alpha_{s+1}+\cdots+0\cdot\alpha_m=0,$$
      这是==大组的一个系数不全为零的零组合==，与整体无关矛盾。

      =="给多出来的向量补零系数"这个动作，是把子组的结论抬到大组上的标准手法==，
      **值得单独记住**——后面证明题里还会反复用到。
      至于"部分相关 $\Rightarrow$ 整体相关"，它就是上面这句话的逆否，==不用另证==。
      ...
      **理解方式**：加分量相当于给方程组==增加方程==，
      ...
      ==反向同样不成立==：**无关的截短未必无关**（分量删多了信息就没了），
      **相关的延长未必相关**。
      $\alpha_1=(1,0)$、$\alpha_2=(0,1)$ 无关，
      各删掉第二个分量后成了 $(1)$ 和 $(0)$，==相关了==。

      正因为只有"截短后无关 $\Rightarrow$ 原组无关"这一个方向成立，
      才有了[截短法](#/linear-algebra/vectors/independence?at=truncate)这条捷径——
      ==它只能用来证无关，不能用来证相关==。
    ...
    { t: 'insight', id: 'my-subset-question', title: '我的疑问：无关组里挑几个出来，还是无关吗', c: String.raw`
      这条逻辑看着很简单，但我一开始是当口诀背的，==没想过为什么==。
      自己问了一遍才发现，答案其实一句话就完了：

      ==把子组那个"不全为零的零组合"补上一堆零系数，它就变成了大组的零组合。==

      想通之后有两点对我挺有用：

      1. **"补零系数"不是这一条的专用技巧**，它是==在大小两个向量组之间搬结论的通用动作==。
         以后凡是遇到"部分怎样，整体怎样"，先想想能不能补零系数把式子撑到大组上去。
      2. **这四条口诀其实只是两条**。
         「部分相关 $\Rightarrow$ 整体相关」和「整体无关 $\Rightarrow$ 部分无关」==互为逆否==，
         「相关的缩短仍相关」和「无关的延长仍无关」也==互为逆否==。
         ==我以前是当四条独立的东西背的，所以老记反==；
         按"证一条、逆否一条"来记，就基本不会串了。

      顺带一个提醒给自己：==别把"删向量"和"删分量"混成一件事==。
      前者是取子组，后者是截短，==结论方向刚好不一样==。
    ` },

    { t: 'method', id: 'truncate', title: '★ 截短法（降阶法）：证高维向量组无关的捷径', c: String.raw`
      **适用场合**：==向量的维数 $m$ 明显大于向量的个数 $n$==
      （比如 $3$ 个 $5$ 维向量），要判断的是==无关==。

      $$\boxed{\ \text{挑出 }n\text{ 行凑成 }n\text{ 阶方阵，}\ \abs{\ \cdot\ }\ne0
      \ \Longrightarrow\ \text{原向量组无关}\ }$$

      **三步**：

      1. 从 $m$ 个分量位置里==任选 $n$ 个==（对每个向量选同样的位置），
         把截短后的 $n$ 个 $n$ 维向量按列拼成方阵；
      2. 算这个 $n$ 阶行列式；
      3. ==不为零==，则截短后的向量组无关，
         再由[「无关的延长仍无关」](#/linear-algebra/vectors/independence?at=properties)
         回推==原向量组无关==。

      **它为什么成立**：截短相当于把方程组的一部分方程拿出来看，
      ==这部分方程都只有零解了，全部方程放在一起当然更只有零解==。

      **用秩来说是同一件事**：$\rank(A)=n\iff A$ ==存在一个 $n$ 阶非零子式==
      （[秩的子式定义](#/linear-algebra/matrix/rank?at=def-rank)）。
      截短法就是==直接去找这个非零子式==，而不去化阶梯形。
      ...
      |---|---|
      | 某个 $n$ 阶行列式 ==$\ne0$== | ==原组无关==（收工） |
      | 某个 $n$ 阶行列式 ==$=0$== | ==什么都得不到==，换一组行再试 |
      | ==所有== $n$ 阶子式都为 $0$ | 这时才能断定==相关== |

      **所以：$\ne0$ 一次就够，$=0$ 一次不算数。**
      ==挑到一个零行列式就说"相关"，是这个方法最常见的错用。==

      **什么时候不要用它**：

      - $m$ 与 $n$ 差不多大（尤其 $m=n$）时，==直接算那一个行列式即可==，
        谈不上"截短"；
      - 题目还要==求极大无关组、求表示式、求秩的具体值==时，
        ==老老实实化阶梯形==，一次行变换能把这些一起回答，
        见[求极大无关组的四步](#/linear-algebra/vectors/basis?at=find-max)。
    ` },
    ...
    { t: 'example',
      id: 'ex-truncate',
      title: '★ 截短法：挑到零行列式说明不了任何事',
      source: '方法示范',
      level: 2,
      problem: String.raw`
        判断
        $$\alpha_1=\begin{pmatrix}1\\1\\1\\1\end{pmatrix},\qquad
        \alpha_2=\begin{pmatrix}1\\1\\2\\3\end{pmatrix},\qquad
        \alpha_3=\begin{pmatrix}1\\1\\3\\6\end{pmatrix}$$
        是否线性无关。
      `,
      idea: String.raw`
        **先看形状**：$3$ 个 $4$ 维向量，==个数 $<$ 维数==，
        所以["个数 $>$ 维数必相关"](#/linear-algebra/vectors/independence?at=judge)==用不上==，
        必须真判断。

        **维数比个数大，正是[截短法](#/linear-algebra/vectors/independence?at=truncate)的主场**：
        只要能挑出 $3$ 行凑成一个非零的三阶行列式就赢了，
        ==比化 $4$ 行的阶梯形省事==。

        **先试最自然的前三行**：
        $$\begin{pmatrix}1&1&1\\ 1&1&1\\ 1&2&3\end{pmatrix}$$
        ==前两行一模一样==（因为三个向量的第 $1,2$ 个分量都是 $1$），行列式为零。

        ==这里是最关键的一步==：**行列式为零，不能下任何结论**。
        截短法只有"$\ne0\Rightarrow$ 无关"这一个方向，
        零行列式只说明==这三行挑得不好==，换一组再试。

        **换哪三行**：既然第 $1,2$ 行重复，==留一个就够了==，
        再补上第 $3,4$ 行，即挑第 $1,3,4$ 行。
      `,
      solution: String.raw`
        取三个向量的第 $1,3,4$ 个分量，截短成
        $$\tilde\alpha_1=\begin{pmatrix}1\\1\\1\end{pmatrix},\qquad
        \tilde\alpha_2=\begin{pmatrix}1\\2\\3\end{pmatrix},\qquad
        \tilde\alpha_3=\begin{pmatrix}1\\3\\6\end{pmatrix}.$$

        计算行列式：
        $$\begin{vmatrix}1&1&1\\ 1&2&3\\ 1&3&6\end{vmatrix}
        \xrightarrow{r_2-r_1,\ r_3-r_1}
        \begin{vmatrix}1&1&1\\ 0&1&2\\ 0&2&5\end{vmatrix}
        =1\times(1\times5-2\times2)=1\ne0.$$

        故 $\tilde\alpha_1,\tilde\alpha_2,\tilde\alpha_3$ ==线性无关==。
        由==无关的延长仍无关==，原向量组 $\alpha_1,\alpha_2,\alpha_3$ ==线性无关==。

        **答题时这句话不能省**：必须写明"截短后无关，故延长后的原向量组无关"，
        ==否则从三维的结论跳到四维是没有依据的==。
      `,
      comment: String.raw`
        **这道题真正要练的不是计算，是那个"零行列式"**。
        前三行算出 $0$ 的那一刻，==如果顺手写下"故线性相关"，这题就全错了==。

        **单向性再强调一次**：

        $$\text{某个三阶子式}\ne0\ \Longrightarrow\ \text{无关}\qquad(\text{一个就够})$$
        $$\text{某个三阶子式}=0\ \Longrightarrow\ \text{什么也得不到}$$
        $$\text{**所有**三阶子式}=0\ \Longrightarrow\ \text{相关}$$

        ==前两句是天壤之别，中间那句才是命题人埋的坑。==
        本题一共有 $C_4^3=4$ 组行可挑，要断定"相关"得四个全算，
        ==而断定"无关"只要撞对一个==。

        **怎么挑行更容易撞对**：

        - ==避开明显重复或成比例的行==（本题的第 $1,2$ 行）；
        - ==优先挑数字差异大的行==，更容易凑出非零行列式；
        - 挑完发现能化成上三角就最好，行列式一眼可得。

        **和化阶梯形比**：本题化 $4\times3$ 的阶梯形当然也行，
        ==但截短法只算了一个三阶行列式==。
        不过==一旦题目还问"求极大无关组"或"把某个向量表示出来"，
        截短法就不够用了==，必须回到行变换。

        **一个常见变体**：再添一个 $\alpha_4$ 使个数 $=$ 维数 $=4$，
        这时==直接算那个四阶行列式==即可，
        ==谈不上挑行，也就没有这个坑了==。
      `,
    },
    ...
    { t: 'warn', id: 'pitfall-list', title: '这一节的固定失分点', c: String.raw`
      ...
      8. **把"删向量"和"删分量"混成一件事**：==前者是取子组，后者是截短==，
         两条口诀管的不是同一个方向。
      9. **截短法挑到一个零行列式就说"相关"**：
         ==$\ne0$ 一次就够，$=0$ 一次不算数==，必须所有 $n$ 阶子式全为零才能断定相关。
      10. **截短后忘了写"延长仍无关"**：==从低维的结论跳回原向量组，必须有这句依据==。
      11. **忘了"个数 $>$ 维数必相关"**：这条能秒杀不少选择题。
      12. **判断可表示时只看 $\rank(A)$**：==必须比较 $\rank(A)$ 与 $\rank(A\mid\beta)$==。
      13. **含零向量的组当成可能无关**：==必相关==。
    ` },
